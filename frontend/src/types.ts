...
export interface DashboardSummary { active_deals:number; monthly_amount:string; machines_count:number; integration_1c_deals:number; funnels:FunnelSummary[]; responsibles:ResponsibleSummary[] }
...
export interface Calculation { id:string; employee_id:string; period_from:string; period_to:string; month:string; version:number; status:string; rules_version:number|null; implementation_total:string; tech_integration_total:string; support_hours:string; sales_total:string; training_count:number; subtotal_dividable:string; cr_start_fixed_total:string; total_bonus:string; issues_count:number; created_at:string }
export interface CalculationItem { id:string; calculation_id:string; employee_id:string; deal_id:string|null; bonus_type:string; source_type:string; source_external_id:string|null; base_amount:string; rate:string; quantity:string; amount_before_divider:string; divider_applied:boolean; amount_final:string; description:string; details_json:string }
