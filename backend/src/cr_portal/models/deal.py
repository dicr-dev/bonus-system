...
class Deal(Base):
    ...
    implementation_responsible_user_id: Mapped[UUID|None]=mapped_column(ForeignKey("users.id",ondelete="SET NULL"),nullable=True,index=True)
    sales_bonus_user_id: Mapped[UUID|None]=mapped_column(ForeignKey("users.id",ondelete="SET NULL"),nullable=True,index=True)
    source_deal_bitrix_id: Mapped[int|None]=mapped_column(Integer,nullable=True,index=True)
    ...
    responsible_user: Mapped["User|None"]=relationship("User",foreign_keys=[responsible_user_id],back_populates="deals")
    implementation_responsible_user: Mapped["User|None"]=relationship("User",foreign_keys=[implementation_responsible_user_id],back_populates="implementation_deals")
    sales_bonus_user: Mapped["User|None"]=relationship("User",foreign_keys=[sales_bonus_user_id])
