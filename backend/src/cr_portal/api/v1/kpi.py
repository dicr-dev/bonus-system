...
async def summary(month:str=Query(...),session:AsyncSession=Depends(db_session),user=Depends(current_user)):
    return await kpi_summary(
        session,
        parse_month(month),
...
async def get_plan(month:str=Query(...),session:AsyncSession=Depends(db_session),_user=Depends(current_user)):
...
async def put_plan(data:PlanInput,month:str=Query(...),session:AsyncSession=Depends(db_session),_admin=Depends(admin_user)):
